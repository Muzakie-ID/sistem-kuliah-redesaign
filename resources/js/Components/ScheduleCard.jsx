...
// Badge status waktu kelas: LIVE (sisa kelas) / "Mulai dalam X" (belum mulai) / "Selesai".
// Hanya aktif untuk jadwal hari ini (isToday). ponytail: pakai waktu lokal browser, bukan offset Asia/Jakarta eksplisit.
function LiveBadge({ start, end, disabled, isToday }) {
    ...
    if (disabled || !isToday) return null;
    ...
    if (ts >= startAt && ts <= endAt) {
        ...
        return (
            <span
                ...
                role="status"
                aria-label={`Sedang berlangsung, sisa ${hms(sisa)}`}
            >
                ...
                LIVE
                <span className="font-mono tabular-nums normal-case tracking-normal">sisa {hms(sisa)}</span>
            </span>
        );
    }

    // Belum mulai: hitung mundur sampai kelas dimulai
    if (ts < startAt) {
        const menuju = startAt - ts;
        return (
            <span
                className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-primary-500/10 border border-primary-500/30 text-primary-700 dark:text-primary-300 text-[11px] font-extrabold uppercase tracking-wider"
                role="timer"
                aria-label={`Kelas dimulai dalam ${hms(menuju)}`}
            >
                <Clock className="w-3 h-3 shrink-0" />
                <span className="font-mono tabular-nums normal-case tracking-normal">Mulai dalam {hms(menuju)}</span>
            </span>
        );
    }
 ...
 */
export default function ScheduleCard({ schedule, onManage, manageLabel = 'Edit Status', onEdit, onDelete, isToday = false }) {
        ...
        <article className={`relative overflow-hidden rounded-3xl border p-4 pl-5 transition-all shadow-card bg-card ${
            ...
            <div className="flex items-start justify-between gap-2 mb-2.5">
                <div className="flex items-center gap-1.5 flex-wrap gap-y-1">
                    <LiveBadge start={schedule.start_time} end={schedule.end_time} disabled={isCancelled} isToday={isToday} />
